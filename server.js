// server.js - Facilitaki Backend Corrigido (Token Fix)
...
const authenticateToken = (req, res, next) => {
    ...
    console.log('🔍 Verificando token:', token ? 'Token presente' : 'Sem token');
...
const authenticateAdmin = (req, res, next) => {
    ...
    console.log('🔍 Verificando token admin:', token ? 'Token presente' : 'Sem token');
    ...
    jwt.verify(token, SECRET_KEY, (err, decoded) => {
        ...
        if (!decoded.isAdmin) {
            console.log('❌ Usuário não é admin:', decoded);
    ...
    try {
        ...
        res.send(`
            ...
            <head>
                ...
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .tab { padding: 10px 20px; background: #ddd; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; transition: all 0.3s ease; }
                    ...
                    .btn { padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer; margin: 2px; transition: all 0.3s ease; }
                    .btn-view { background: #3498db; color: white; }
                    .btn-update { background: #2ecc71; color: white; }
                    .btn-delete { background: #e74c3c; color: white; }
                    .logout-btn { background: #e74c3c; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; }
                    .admin-badge { background: #667eea; color: white; font-size: 10px; padding: 2px 6px; border-radius: 10px; margin-left: 5px; }
                    .create-admin-section { margin-top: 20px; padding: 20px; background: #f8f9fa; border-radius: 10px; border: 1px solid #e1e5e9; }
                    .flex-row { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
                    .flex-row input { flex: 2; padding: 10px; border: 1px solid #ddd; border-radius: 5px; }
                    .flex-row button { background: #667eea; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; }
            ...
            <body>
                ...
                <div class="stats">
                    <div class="stat-card"><div class="stat-number">${pedidos.rows.length}</div><div><i class="fas fa-shopping-cart"></i> Total Pedidos</div></div>
                    <div class="stat-card"><div class="stat-number">${pedidos.rows.filter(p => p.status === 'pendente').length}</div><div><i class="fas fa-clock"></i> Pendentes</div></div>
                    <div class="stat-card"><div class="stat-number">${usuarios.rows.filter(u => !u.is_admin).length}</div><div><i class="fas fa-users"></i> Clientes</div></div>
                    <div class="stat-card"><div class="stat-number">${contatos.rows.length}</div><div><i class="fas fa-envelope"></i> Contatos</div></div>
                </div>
                ...
                <div id="tab-pedidos" class="tab-content active">
                    ...
                    <tbody>${pedidos.rows.map(p => `<tr><td>${p.id}</td><td>${p.cliente || '-'}</td><td>${p.telefone || '-'}</td><td>${p.nome_plano || p.plano || '-'}</td><td>${parseFloat(p.preco || 0).toLocaleString('pt-MZ')} MT</td><td><span class="status status-${p.status || 'pendente'}">${p.status || 'pendente'}</span></td><td>${p.data_pedido ? new Date(p.data_pedido).toLocaleDateString() : '-'}</td><td><button class="btn btn-view" onclick="viewPedido(${p.id})"><i class="fas fa-eye"></i></button><button class="btn btn-update" onclick="updateStatus(${p.id})"><i class="fas fa-edit"></i></button><button class="btn btn-delete" onclick="deletePedido(${p.id})"><i class="fas fa-trash"></i></button></td></tr>`).join('')}</tbody>
                    </table>
                </div>
                
                <div id="tab-usuarios" class="tab-content">
                    <table><thead><tr><th>ID</th><th>Nome</th><th>Telefone</th><th>Admin</th><th>Cadastro</th><th>Ações</th></tr></thead>
                    <tbody>${usuarios.rows.map(u => `<tr><td>${u.id}</td><td>${u.nome} ${u.is_admin ? '<span class="admin-badge">Admin</span>' : ''}</td><td>${u.telefone}</td><td>${u.is_admin ? 'Sim' : 'Não'}</td><td>${new Date(u.created_at).toLocaleDateString()}</td><td>${!u.is_admin ? `<button class="btn btn-delete" onclick="deleteUser(${u.id})"><i class="fas fa-trash"></i></button>` : ''}</td></tr>`).join('')}</tbody>
                    </table>
                </div>
                ...
                <div id="tab-contatos" class="tab-content">
                    ...
                    <tbody>${contatos.rows.map(c => `<tr><td>${c.id}</td><td>${c.nome}</td><td>${c.telefone}</td><td>${c.mensagem.substring(0, 50)}${c.mensagem.length > 50 ? '...' : ''}</td><td>${new Date(c.data_envio).toLocaleDateString()}</td><td><button class="btn btn-view" onclick="viewContato(${c.id})"><i class="fas fa-eye"></i></button><button class="btn btn-delete" onclick="deleteContato(${c.id})"><i class="fas fa-trash"></i></button></td></tr>`).join('')}</tbody>
                    </table>
                </div>
                ...
                <div id="tab-admins" class="tab-content">
                    <div class="create-admin-section">
                        ...
                        <div class="flex-row">
                            <input type="text" id="newAdminUser" placeholder="Usuário">
                            <input type="password" id="newAdminPass" placeholder="Senha">
                            <input type="password" id="newAdminPassConfirm" placeholder="Confirmar Senha">
                            <button onclick="createNewAdmin()"><i class="fas fa-user-plus"></i> Criar Admin</button>
                        </div>
                        <div id="adminCreateMessage" style="margin-top: 10px;"></div>
                    </div>
                    <h4 style="margin: 20px 0 10px;">Administradores Existentes</h4>
                    <div id="adminsList"></div>
                </div>
                
                <div id="modal" class="modal"><div class="modal-content"><div id="modalBody"></div><button onclick="closeModal()" style="margin-top:15px; padding:8px 16px; background:#6c757d; color:white; border:none; border-radius:5px;">Fechar</button></div></div>
                ...
                <script>
                    ...
                    async function loadAdmins() {
                        ...
                        if (data && data.success) {
                            ...
                            if (data.admins.length === 0) {
                                ...
                                container.innerHTML = '<table><thead><tr><th>ID</th><th>Nome</th><th>Data de Criação</th><th>Ações</th></tr></thead><tbody>' + data.admins.map(admin => '<tr><td>' + admin.id + '</td><td>' + admin.nome + ' <span class="admin-badge">Admin</span>' + (admin.id === ${req.admin.id} ? ' (Você)' : '') + '</td><td>' + new Date(admin.created_at).toLocaleDateString() + '</td><td>' + (admin.id !== ${req.admin.id} ? '<button class="btn btn-delete" onclick="deleteAdmin(' + admin.id + ')"><i class="fas fa-trash"></i></button>' : '<span style="color:#999;">Não pode excluir próprio</span>') + '</td></tr>').join('') + '</tbody></table>';
                            }
                        }
                    }
                    ...
                    async function createNewAdmin() {
                        ...
                        const data = await apiCall('/api/admin/register', { method: 'POST', body: JSON.stringify({ usuario, senha }) });
                    ...
                    async function viewPedido(id) {
                        ...
                        if (data && data.success) {
                            document.getElementById('modalBody').innerHTML = '<h3>Pedido #' + data.pedido.id + '</h3><p><strong>Cliente:</strong> ' + (data.pedido.cliente || '-') + '</p><p><strong>Telefone:</strong> ' + (data.pedido.telefone || '-') + '</p><p><strong>Serviço:</strong> ' + (data.pedido.nome_plano || data.pedido.plano || '-') + '</p><p><strong>Valor:</strong> ' + parseFloat(data.pedido.preco || 0).toLocaleString('pt-MZ') + ' MT</p><p><strong>Status:</strong> ' + (data.pedido.status || 'pendente') + '</p><p><strong>Descrição:</strong> ' + (data.pedido.descricao || 'Nenhuma') + '</p>' + (data.pedido.arquivo_path ? '<p><strong>Arquivo:</strong> <a href="/' + data.pedido.arquivo_path + '" target="_blank">Download</a></p>' : '');
                    ...
                    async function viewContato(id) {
                        ...
                        if (data && data.success) {
                            document.getElementById('modalBody').innerHTML = '<h3>Contato #' + data.contato.id + '</h3><p><strong>Nome:</strong> ' + data.contato.nome + '</p><p><strong>Telefone:</strong> ' + data.contato.telefone + '</p><p><strong>Data:</strong> ' + new Date(data.contato.data_envio).toLocaleString() + '</p><p><strong>Mensagem:</strong></p><p style="margin-top:10px; padding:10px; background:#f5f5f5; border-radius:5px;">' + data.contato.mensagem + '</p>';
                            document.getElementById('modal').style.display = 'flex';
                        }
                    }
...
app.put('/api/admin/pedido/:id/status', authenticateAdmin, async (req, res) => {
    try {
        ...
        if (!validStatus.includes(status)) return res.status(400).json({ success: false, error: 'Status inválido' });
